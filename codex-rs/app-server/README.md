...
- `thread/settings/update` — experimental; queue a partial update to a loaded thread’s next-turn settings without starting a turn or adding transcript items. Omitted fields leave settings unchanged; `serviceTier: null` clears the tier; `runtimeWorkspaceRoots` replaces the thread-scoped runtime roots used to materialize `:workspace_roots`, resolving relative paths against the effective cwd. Runtime roots are limited to 100 entries, 4096 characters per resolved root, and 1 MiB of aggregate request path text. `sandboxPolicy` and `permissions` cannot be combined. Returns `{}` when the update is accepted and emits `thread/settings/updated` with the full effective settings only if they actually change. `turn/start` settings overrides emit the same notification when they change the stored settings.
...
Experimental clients can store a later submission with `thread/queue/add`. The queue item keeps the message and its turn-scoped context. Dispatch uses the thread's current settings. Clients render queue previews from bounded server snapshots and use paginated `thread/queue/list` when they need the full queue.
