...
async fn app_server_emits_structured_tool_timing_events() -> Result<()> {
    ...
    let dispatch_duration = duration_field(&tool_call, "dispatch_duration_seconds");
    let handler_duration = duration_field(&tool_call, "handler_duration_seconds");
    let total_duration = duration_field(&tool_call, "total_duration_seconds");
    ...
    assert_nonnegative_duration_fields(&tool_result, &["duration_seconds"]);
    ...
    for inference in inferences {
        ...
        assert_nonnegative_duration_fields(&inference, &["duration_seconds"]);
    }
