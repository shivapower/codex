...
const RECOMMENDED_PLUGINS_INTRO: &str = "Here is a list of plugins that are available but not installed. If the user's query would benefit from one or more of these plugins, use the `request_plugin_installs` tool to suggest installing them. Pass `action_type` as `install` and include each parenthesized ID in `entries` as `tool_id` with `tool_type` set to `plugin`. For example, suggest the Google Drive plugin if the query could possibly be better answered with access to Google Drive.";
